...
public class AspectRatioConcordanceManager extends AbstractConcordanceManager<AspectRatioConcordanceBean> {
	...
	private static final String CONCORANCE_FILE_NAME = "/aspect_ratio.csv";
     ...
     */
	public AspectRatioConcordanceManager() throws URISyntaxException, IOException {
	 ...
	 * @throws FileNotFoundException - If file not found on classpath.
	 */
	public AspectRatioConcordanceManager(String fileName) throws URISyntaxException, IOException {
		super(fileName);
	}
	 ...
	 */
	@Override
	protected Map<String, AspectRatioConcordanceBean> initBeanMap(
			HeaderColumnNameTranslateMappingStrategy<AspectRatioConcordanceBean> strat,
			CSVReader reader) {
		
	    CsvToBean<AspectRatioConcordanceBean> csv = new CsvToBean<>();
	    List<AspectRatioConcordanceBean> list = csv.parse(strat, reader);
	    // populate local map of keyword name to Bean
	    Map<String, AspectRatioConcordanceBean> map = new TreeMap<>(String.CASE_INSENSITIVE_ORDER); // could be looking up with keys in mixed case
	    for (AspectRatioConcordanceBean item : list) {
	 ...
	 */
	@Override
	protected Class<AspectRatioConcordanceBean> getBeanClass() {
		return AspectRatioConcordanceBean.class;
	}
