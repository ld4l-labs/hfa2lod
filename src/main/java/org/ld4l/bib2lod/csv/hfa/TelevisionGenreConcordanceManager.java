...
public class TelevisionGenreConcordanceManager extends AbstractConcordanceManager<TelevisionGenreConcordanceBean> {
	 ...
	 * @throws FileNotFoundException - If file not found on classpath.
	 */
	protected TelevisionGenreConcordanceManager(String fileName) throws URISyntaxException, IOException {
		super(fileName);
	}
	 ...
	 */
	@Override
	protected Map<String, TelevisionGenreConcordanceBean> initBeanMap(
			HeaderColumnNameTranslateMappingStrategy<TelevisionGenreConcordanceBean> strat,
			CSVReader reader) {
		
	    CsvToBean<TelevisionGenreConcordanceBean> csv = new CsvToBean<>();
	    List<TelevisionGenreConcordanceBean> list = csv.parse(strat, reader);
	    // populate local map of keyword name to Bean
	    Map<String, TelevisionGenreConcordanceBean> map = new HashMap<>(list.size());
	    for (TelevisionGenreConcordanceBean item : list) {
